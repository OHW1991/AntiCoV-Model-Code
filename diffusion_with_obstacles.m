function [hitRate] = diffusion_with_obstacles(N, D, dt, totalTime, wallPos, K, obstacleSize, attachmentTime)
    ...
    obstaclePositions = normrnd(0, 10, K, 3); % Obstacle positions normally distributed in space
    ...
    for t = 1:numSteps
        ...
        x(~attached) = x(~attached) + dx(~attached);
        y(~attached) = y(~attached) + dy(~attached);
        z(~attached) = z(~attached) + dz(~attached);

        % Check for particles hitting the wall
        hitIndices = x < wallPos & ~attached;
        ...
        for i = 1:N
            if ~attached(i)
                for j = 1:K
                    if norm([x(i) y(i) z(i)] - obstaclePositions(j, :)) < obstacleSize
                        attached(i) = true;
