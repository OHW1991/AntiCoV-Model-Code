function [hitRate,particleCounter,deathCounter,infectionCounter] = diffusion_with_obstacles_and_hit_count(N, D, dt, totalTime, wallPos, K, obstacleSize, attachmentTime, spaceSize, H, P)
    ...
    % H             - Maximal hit count for particles
    % P             - Number of new particles to appear when a particle with H-1 hits the wall
    ...
    % (x=0)
%     obstaclePositions(:,1) =  0.1 * spaceSize(1) + 0.75 * spaceSize(1) .* rand(K, 1);
    ...
    % Initialize particle attachment status and hit counts
    attached = false(N, 1);
    ...
    particleCounter = zeros(numSteps,1);
    ...
    for t = 1:numSteps
        ...
        if any(hitIndices)
            for i = find(hitIndices)'
                hitCount(i) = hitCount(i) + 1;
                if hitCount(i) >= H
                    infectionCounter = infectionCounter+1;
                    % Particle disappears and P random particles appear
                    hitCount(i) = NaN; % Mark particle for removal
                    % Generate a random number of new particles based on
                    % the value of P
                    newParticleNum = P;
%                     newX = abs(normrnd(spaceSize(1)/10, spaceSize(1)/20, newParticleNum, 1));
                    newX = rand(newParticleNum, 1) * 200 + 50; %spaceSize(1);
                    newY = rand(newParticleNum, 1) * spaceSize(2);
                    newZ = rand(newParticleNum, 1) * spaceSize(3);
                    x = [x; newX];
                    y = [y; newY];
                    z = [z; newZ];
                    N = N+newParticleNum-1;
                    attached = [attached; false(newParticleNum, 1)];
                    nonAttached = [nonAttached; true(newParticleNum, 1)];
                    attachmentCounter = [attachmentCounter; zeros(newParticleNum, 1)];
                    hitCount = [hitCount; zeros(newParticleNum, 1)];
                end
            end
            hits = hits + nnz(hitIndices);
        end

        % Reflect particles that hit the measurement wall but keep them in
        % proximity
%         x(hitIndices) = -x(hitIndices)*randi(10);
        x(hitIndices) = randi([250,1000]);
        ...
        if any(hitObstacles)
            for i = find(hitObstacles)'
                ...
                if hitCount(i) >= H
                    ...
                    attached(i) = true;
                    attachmentCounter(i) = attachmentTime;
                end
            end
        end
        ...
        removeIndices = isnan(hitCount);
        deathCounter = deathCounter+ sum(removeIndices);
        x(removeIndices) = [];
        y(removeIndices) = [];
        z(removeIndices) = [];
        attached(removeIndices) = [];
        nonAttached(removeIndices) = [];
        attachmentCounter(removeIndices) = [];
        hitCount(removeIndices) = [];
        
        
        % Spontaneously reduce hitcount for particles which are not
        % attached
        temp = datasample([0,0,0,0,1],numel(hitCount(nonAttached)));
        hitCount(nonAttached) = max(0, hitCount(nonAttached)-temp');
        ...
        particleCounter(t,1) = N;
        
%         scatter3(x,y,z); hold on;
%         scatter3(obstaclePositions(:,1),obstaclePositions(:,2),obstaclePositions(:,3),100,'filled');
...
%         view(3);
%         hold off;
%         xlim([0,spaceSize(1)]); ylim([0,spaceSize(2)]); zlim([0,spaceSize(3)]);
%         xlabel('X');
%         ylabel('Y');
%         zlabel('Z');
%         fprintf('something\n');
